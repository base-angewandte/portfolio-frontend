build-app:
	docker run -it -v "$(shell pwd)"/dist:/app/dist_host baseangewandte/portfolio-frontend-build:1.1.1 sh -c "npm run build && rm -rf /app/dist_host/* && mv /app/dist/* /app/dist_host"
